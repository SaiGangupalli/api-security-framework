...
class DocumentGenerator:
    ...
    def _clean_text_content(self, text: str) -> str:
        ...
        text = re.sub(r'^\s+|\s+$', '', text, flags=re.MULTILINE)  # Remove leading/trailing spaces from each line
    ...
    def _add_consolidated_security_analysis(self, issues_data: List[Dict]):
        ...
        if high_risk_issues:
            ...
            para.add_run('High Risk Issues: ').bold = True
            para.add_run(', '.join(high_risk_issues))
        
        if medium_risk_issues:
            para = self.doc.add_paragraph()
            para.add_run('Medium Risk Issues: ').bold = True
            para.add_run(', '.join(medium_risk_issues))
        
        if low_risk_issues:
            para = self.doc.add_paragraph()
            para.add_run('Low Risk Issues: ').bold = True
            para.add_run(', '.join(low_risk_issues))
    ...
    def generate_html_summary(self, issues_data: List[Dict], executive_summary: str) -> str:
        ...
        # Clean the executive summary for HTML
        clean_exec_summary = self._clean_text_content(executive_summary).replace('\n', '<br>')
        ...
        for issue in issues_data:
            ...
            clean_summary = self._clean_text_content(issue['summary'])
            truncated_summary = clean_summary[:60] + '...' if len(clean_summary) > 60 else clean_summary
            
            html_content += f"""
                <tr class="{risk_class}">
                    <td>{issue['key']}</td>
                    <td>{truncated_summary}</td>
...
class JiraClient:
    ...
    def extract_issue_details(self, issue_key: str) -> Dict:
        ...
        try:
            ...
            # Handle description field (can be ADF format or string)
            description_field = fields.get('description')
            if isinstance(description_field, dict):
                description = self._clean_html(description_field)
            elif isinstance(description_field, str):
                description = self._clean_html(description_field)
            else:
                description = self._clean_html(str(description_field) if description_field else "")
    ...
    def _clean_html(self, text) -> str:
        ...
        try:
            ...
            if cleaned_text:
                ...
                cleaned_text = re.sub(r'^\s+|\s+$', '', cleaned_text, flags=re.MULTILINE)  # Leading/trailing spaces per line
